...
export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="platform-page service-detail"><SiteHeader current="services" />
    ...
    <section className="service-detail-hero"><div><span>{service.label}</span><h1>{service.title}</h1><p>{service.description}</p><div className="hero-actions"><a className="button button--primary" href="/contact">このサービスについて相談 <b>→</b></a><a className="button button--secondary" href="/services#service-menu">サービス一覧へ <b>→</b></a></div></div><dl><div><dt>料金の目安</dt><dd>{service.price}</dd></div><div><dt>期間の目安</dt><dd>{service.period}</dd></div></dl></section>
