...
export default function Home() {
  return <main>
    ...
    <section className="section home-column home-column--latest"><div className="section-heading"><div><p className="section-kicker">LATEST COLUMN</p><h2>最新コラム</h2></div><a href="/column">コラムをすべて見る <span>→</span></a></div><div className="column-card-grid column-card-grid--home">{columnItems.slice(0, 3).map((item) => <article className="column-card" key={item.slug}><a className="column-card-visual" href={`/column/${item.slug}`} style={{ backgroundImage:`url(${item.image})` }} aria-label={`${item.title}を読む`}><span>{item.category}</span></a><div className="column-card-body"><div><time>{item.date}</time><span>SHORT VIDEO</span></div><p className="column-card-hook">{item.videoHook}</p><h3><a href={`/column/${item.slug}`}>{item.title}</a></h3><p>{item.summary}</p><a className="text-link" href={`/column/${item.slug}`}>続きを読む <span>→</span></a></div></article>)}</div></section>

    <section className="section home-knowledge-section" id="knowledge"><div className="section-heading"><div><p className="section-kicker">KNOWLEDGE</p><h2>ナレッジ</h2></div><a href="/knowledge#articles">ナレッジをすべて見る <span>→</span></a></div><HomeKnowledgeTabs /></section>

    <section className="section home-basics-section" id="basics"><div className="section-heading"><div><p className="section-kicker">BASIC KNOWLEDGE</p><h2>基礎知識</h2></div></div><ol className="knowledge-path-grid home-knowledge-path-grid">{guideSteps.map((step) => <li key={step.slug}><a href={`/knowledge/${step.slug}`}><span>{step.number}</span><div><small>STEP {step.number}</small><h3>{step.title}</h3><p>{step.description}</p><em>{step.articles.length}テーマ</em></div><b>→</b></a></li>)}</ol></section>
