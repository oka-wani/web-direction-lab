...
export default function HearingForm(){
  ...
  useEffect(()=>{if(!formRef.current||!token)return;const saved=localStorage.getItem(`wsw-hearing-v3:${token}`);if(!saved)return;try{const record=JSON.parse(saved) as Record<string,string[]>;for(const [name,values] of Object.entries(record)){const elements=Array.from(formRef.current.querySelectorAll(`[name="${CSS.escape(name)}"]`)) as FormControl[];elements.forEach(el=>{if(el instanceof HTMLInputElement&&(el.type==="checkbox"||el.type==="radio"))el.checked=values.includes(el.value);else if(values[0]!=null)el.value=values[0];});}}catch{}},[token]);
  ...
  return <form ref={formRef} className="hearing-wizard" onSubmit={submit} onChange={save}>
    ...
    <section data-step="3" hidden={step!==3} className="hearing-step"><small>約2分</small><div className="hearing-step-title"><h2>目的・導線</h2><span>* 必須項目</span></div>
      <Hint>目的から、サイトの導線を提案します。主目的と事業内容をもとに、問い合わせ・予約などのメインCTAはこちらで設定します。</Hint>
    ...
    <section data-step="4" hidden={step!==4} className="hearing-step"><small>約3分</small><div className="hearing-step-title"><h2>構成・デザイン</h2><span>* 必須項目</span></div>
      ...
      <Field q="Q14" title="ページ構成・情報の見せ方で参考にしたいサイト" optional><textarea name="structureReference" rows={4} placeholder={'https://example.com\nサービス説明から問い合わせまでの流れが分かりやすい'}/></Field>
      <Field q="Q15" title="希望するサイトの印象" required description="最大3つまで選択してください。">{choices("impression",impressions,"checkbox",3)}</Field>
      <div className="hearing-inline-fields"><Field q="Q16" title="使いたい色" optional><input name="useColor" placeholder="例：深い青、白"/></Field><Field q="" title="避けたい色" optional><input name="avoidColor" placeholder="例：赤、蛍光色／特になし"/></Field></div>
      <Field q="Q17" title="色・雰囲気・デザインで参考にしたいサイト" optional><textarea name="designReference" rows={4} placeholder={'https://example.com\n余白の使い方と落ち着いた色合いが好み'}/></Field>
    ...
    <section data-step="5" hidden={step!==5} className="hearing-step"><small>約2分</small><div className="hearing-step-title"><h2>機能・運用</h2><span>* 必須項目</span></div>
      <Field q="Q19" title="CMSを導入しますか？" required>{choices("cms",["導入したい","不要","相談して決めたい"],"radio")}</Field>
    ...
    <section data-step="6" hidden={step!==6} className="hearing-step hearing-review"><small>確認</small><div className="hearing-step-title"><h2>回答内容をご確認ください。</h2><span>* 最終確認</span></div><dl>{step===6&&values().map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><label className="form-consent"><input type="checkbox" name="consent" value="agreed" required/><span><a href="/about#privacy" target="_blank">個人情報の取り扱い</a>に同意する <b>必須</b></span></label><div className="form-turnstile"><div ref={turnstileRef}/>{turnstileStatus==="loading"&&<p>Bot確認を行っています。</p>}{turnstileStatus==="error"&&<p>Bot確認を表示できませんでした。ページを再読み込みしてください。</p>}</div></section>

    {status==="error"&&<p className="form-notice" role="alert">{message}</p>}
    <div className="hearing-wizard-actions">{step>1&&<button type="button" className="hearing-back" onClick={back}>戻る</button>}<button type={step===6?"submit":"button"} className="button button--primary" onClick={step===6?undefined:next} disabled={status==="sending"}>{status==="sending"?"送信中…":step===6?"この内容で送信する":"次へ進む"}<b>→</b></button></div>
  </form>;
}

function Field({q,title,required,optional,description,children}:{q:string;title:string;required?:boolean;optional?:boolean;description?:string;children:React.ReactNode}){return <div className="hearing-field"><div className="hearing-field-label"><b>{q}</b><strong>{title}</strong>{required&&<em>必須</em>}{optional&&<span>任意</span>}</div>{description&&<p className="hearing-field-description">{description}</p>}{children}</div>}
function Hint({children}:{children:React.ReactNode}){return <p className="hearing-proposal-hint"><b>提案への反映：</b>{children}</p>}
