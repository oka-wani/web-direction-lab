...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (input.url) {
    try {
      ...
      return json(400, "対象サイトURLの形式を確認してください。");
    }
  }
  ...
  const verification = await fetch("https://challenges.cloudflare.com/turnstile/v0/siteverify", {
    ...
    body: JSON.stringify({ secret: env.TURNSTILE_SECRET_KEY, response: token, remoteip: request.headers.get("CF-Connecting-IP"), idempotency_key: crypto.randomUUID() }),
  });
  ...
  const rows = [
    ["氏名", input.name],
    ["メールアドレス", input.email],
    ["会社名・屋号", input.company || "未入力"],
    ["対象サイトURL", input.url || "未入力"],
    ["問い合わせ種別", input.service],
    ["相談内容", input.message || "未入力"],
  ];
  ...
  const customerText = shouldGuideHearing
    ? `${input.name}様\n\nお問い合わせありがとうございます。以下の内容で受け付けました。\n内容を確認のうえ、担当よりご連絡いたします。\n\nWebサイトの制作・リニューアルをご希望の場合は、事前に以下のヒアリングフォームへ分かる範囲でご回答ください。\n\n${HEARING_URL}\n\n--- お問い合わせ内容 ---\n${text}`
    : `${input.name}様\n\nお問い合わせありがとうございます。以下の内容で受け付けました。\n内容を確認のうえ、担当よりご連絡いたします。\n\n--- お問い合わせ内容 ---\n${text}`;

  const customerHtml = shouldGuideHearing
    ? `<p>${escapeHtml(input.name)}様</p><p>お問い合わせありがとうございます。以下の内容で受け付けました。<br>内容を確認のうえ、担当よりご連絡いたします。</p><p><strong>制作ヒアリング</strong><br>Webサイトの制作・リニューアルをご希望の場合は、事前に分かる範囲でご回答ください。</p><p><a href="${HEARING_URL}">${HEARING_URL}</a></p><hr><h2 style="font-size:18px">お問い合わせ内容</h2>${html}`
    : `<p>${escapeHtml(input.name)}様</p><p>お問い合わせありがとうございます。以下の内容で受け付けました。<br>内容を確認のうえ、担当よりご連絡いたします。</p><hr><h2 style="font-size:18px">お問い合わせ内容</h2>${html}`;
    ...
    body: JSON.stringify([
      {
        ...
        subject: `【Wani san Web】${input.name}様からのお問い合わせ`,
        text: `${text}\n\n送信日時: ${sentAt}\n\n初回打ち合わせ候補日:\n${candidateText}\n\nヒアリングURL: ${HEARING_URL}`,
        html: `${html}<p><strong>送信日時</strong><br>${escapeHtml(sentAt)}</p><p><strong>初回打ち合わせ候補日</strong></p><ul>${candidateHtml}</ul><p><strong>ヒアリングURL</strong><br><a href="${HEARING_URL}">${HEARING_URL}</a></p>`,
      },
      {
        from: env.CONTACT_FROM_EMAIL,
        to: [input.email],
        reply_to: ADMIN_EMAIL,
        subject: "【Wani san Web】お問い合わせを受け付けました",
        text: customerText,
        html: customerHtml,
      },
    ]),
  ...
  if (!response.ok) {
    console.error(JSON.stringify({ event: "resend_error", status: response.status, body: await response.text() }));
    return json(502, "メール送信に失敗しました。時間を置いてもう一度お試しください。");
  }
