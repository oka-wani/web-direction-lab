...
const concepts = [
  ...
  { id: "white-grid-monogram", name: "白 B-02｜WSW monogram", note: "大きなWの造形とページレイヤーを融合した、ブランド性の強い案", tone: "white", art: "monogram" },
  ...
  { id: "navy-editorial", name: "濃紺 01｜Bold editorial", note: "濃紺一色と巨大なWSWで、信頼感を強く出す案", tone: "navy" },
...
function BrandIllustration({ type }: { type: BrandArt }) {
  if (type === "layers") return <div className="brand-visual brand-visual--layers" aria-hidden="true">
    ...
    <strong>WSW</strong><small>STRUCTURE / CONNECT / GROW</small>
  </div>;
    ...
    <div className="mv-concept-list">{concepts.map((concept) => <section className={`mv-concept mv-concept--${concept.id}`} data-tone={concept.tone} key={concept.id}>
      ...
      {"art" in concept && concept.art && ["layers", "monogram", "cascade", "rebuild"].includes(concept.art)
        ...
        : "art" in concept && concept.art
          ...
          : <div className="mv-concept-art" aria-hidden="true"><strong>WSW</strong><div className="mv-art-steps"><span><b>01</b> KNOWLEDGE</span><span><b>02</b> ANALYSIS</span><span><b>03</b> ACTION</span></div><div className="mv-art-signal"><i /><i /><i /><i /></div><small>LEARN · IMPROVE · GROW</small></div>}
    </section>)}</div>
    <SiteFooter />
  </main>;
}
