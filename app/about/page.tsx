...
export default function AboutPage() {
  return <main className="about-page"><SiteHeader current="about" /><section className="about-hero"><span className="section-kicker" aria-hidden="true">ABOUT</span><h1>このサイトについて</h1><p>運営方針と、安心して情報を利用していただくための考え方をまとめています。</p></section><div className="about-layout"><aside><p>CONTENTS</p><nav>{sections.map((section)=><a href={`#${section.id}`} key={section.id}><span>{section.number}</span>{section.title}</a>)}</nav></aside><div className="about-content">{sections.map((section)=><section id={section.id} key={section.id}><header><span>{section.number}</span><h2>{section.title}</h2></header>{section.body}</section>)}</div></div><SiteFooter /></main>;
}
