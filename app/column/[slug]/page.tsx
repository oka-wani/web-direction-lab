...
export default async function ColumnDetail({ params }: { params:Promise<{slug:string}> }) {
  ...
  return <main className="column-page"><SiteHeader current="column" /><article className="column-detail"><p className="breadcrumb"><a href="/">トップ</a> / <a href="/column">コラム</a> / {item.category}</p><header><div><span>{item.category}</span><time>{item.date}</time></div><h1>{item.title}</h1><p>{item.lead}</p></header>{item.sections.map((section,index)=><section key={section.title}><small>{String(index+1).padStart(2,"0")}</small><h2>{section.title}</h2><p>{section.body}</p></section>)}<div className="column-back"><a className="button button--secondary" href="/column">← コラム一覧へ戻る</a></div></article><SiteFooter /></main>;
}
