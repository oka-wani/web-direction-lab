...
export const GET:APIRoute=async({params})=>{
  ...
  else if(asset==="proposal.pdf")key=`proposals/${accessId}/proposal.pdf`;
  else if(asset==="proposal-final.pdf")key=`proposals/${accessId}/proposal-final.pdf`;
  ...
  const headers=new Headers();object.writeHttpMetadata(headers);if(asset!=="proposal.pdf"&&asset!=="proposal-final.pdf")headers.set("content-type","text/html; charset=utf-8");headers.set("cache-control","private, no-store");headers.set("x-robots-tag","noindex, nofollow");
