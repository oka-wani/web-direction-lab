...
export default function Home() {
  return <main>
    <SiteHeader />
    <section className="home-main-hero" id="top"><div><p className="eyebrow">LEARN · THINK · APPLY</p><h1>Webを学び、考え、<br /><span>仕事で使える知識</span>に。</h1><p>Webディレクター・Webコンサルを目指す人へ。最新情報、体系的な知識、実践から得た視点を一つにつなげます。</p><div className="hero-actions"><a className="button button--primary" href="/knowledge">Webナレッジで学ぶ <b>→</b></a><a className="button button--secondary" href="/column">コラムを見る <b>→</b></a></div></div></section>

    <section className="section home-column home-column--latest"><div className="section-heading"><div><span className="section-kicker" aria-hidden="true">LATEST COLUMN</span><h2>最新コラム</h2></div><a href="/column">コラムをすべて見る <span>→</span></a></div>{columnItems.length > 0 ? <div className="column-card-grid column-card-grid--home">{columnItems.slice(0, 3).map((item) => <article className="column-card" key={item.slug}><a className="column-card-visual" href={`/column/${item.slug}`} style={{ backgroundImage:`url(${item.image})` }} aria-label={`${item.title}を読む`}><span>{item.category}</span></a><div className="column-card-body"><div><time>{item.date}</time><span>WEB COLUMN</span></div><p className="column-card-hook">{item.videoHook}</p><h3><a href={`/column/${item.slug}`}>{item.title}</a></h3><a className="text-link" href={`/column/${item.slug}`}>続きを読む <span>→</span></a></div></article>)}</div> : <div className="home-empty"><b>新しいコラムを準備中です</b><p>Webの現場やサイト改善に役立つテーマを公開します。</p></div>}</section>
