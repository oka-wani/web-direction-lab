...
export const metadata: Metadata = { title: "標準テンプレート・制作イメージ｜Wani san Web", description: "基本制作パッケージで利用できる、業種別の標準テンプレートとWebサイトの制作イメージをご覧いただけます。" };
...
export default function DemoPage() {
  return <main className="order-page"><SiteHeader current="examples" />
    <section className="order-page-hero"><span>STANDARD TEMPLATES</span><h1>標準テンプレート・<br />制作イメージ</h1><p>基本制作パッケージで利用できる、業種別のページ構成とデザイン例です。実績ではなく、依頼前に構成・機能・デザインの方向性を具体化するための制作イメージです。</p><div className="order-actions"><a className="order-secondary-link" href="/pricing">料金・プランへ戻る <b>→</b></a><a className="order-secondary-link" href="/modules">標準モジュールを見る <b>→</b></a></div></section>
