...
import { articleItems, categories, getKnowledgeCategory, matchesCategory } from "./article-data";
...
export default function ArticlesList() {
  ...
  const shown = useMemo(() => {
    ...
    return knowledgeItems.filter((article) => {
      const displayCategory = getKnowledgeCategory(article);
      const searchableText = [article.title, article.description, displayCategory, ...(article.tags ?? [])].join(" ").toLowerCase();
  ...
  return <>
    <div className="article-tools">
      ...
      <label className="article-search"><span className="sr-only">ナレッジを検索</span><input value={query} onChange={(event) => { setQuery(event.target.value); setVisibleCount(12); }} placeholder="キーワードで検索" /><b aria-hidden="true">⌕</b></label>
    </div>
    ...
    <div className="archive-grid">{visibleItems.map((article) => <article className="archive-card" key={article.slug}>
      <div className="archive-body"><span className="knowledge-category-badge">{getKnowledgeCategory(article)}</span><h2><a href={article.href}>{article.title}</a></h2><p>{article.description}</p><ul className="archive-tags knowledge-term-tags" aria-label="記事内の用語集">{(article.tags ?? []).slice(0, 4).map((tag) => <li key={tag}>#{tag}</li>)}</ul><a className="text-link card-read-link" href={article.href}><span>この記事を読む</span><i>→</i></a></div>
    </article>)}</div>
